...
def dynamic_connected_voronoi(density, max_density, min_density):
    ...
    for k in range(iterations):
        ...
        ax.set_xlabel("X")
        ax.set_ylabel("Y")
        ax.set_title(f"Iteration {k+1} - Dynamic Communication Extension")
    ...
    output_path = os.path.join(repo_root, "data", "last", "dynamic_connected_output.csv")
    with open(output_path, mode="w", newline="") as file:
        ...
        for index, value in enumerate(total_H, start=1):
            writer.writerow([f"Iteration {index}: {value}"])
        writer.writerow([])
        writer.writerow(["Dynamic Communication Extension"])
        writer.writerow([])
