...
def connected_voronoi(density, max_density, min_density):
    ...
    for k in range(iterations):
        ...
        for robot in range(N):
            ...
            ideal_circle = Circle((current_x[robot], current_y[robot]), radius=Rc, fill=False, color="white", linestyle="--")
        ...
        ax.set_xlabel("X")
        ax.set_ylabel("Y")
        ax.set_title(f"Iteration {k+1} - Connectivity Maintenance Baseline")
    ...
    output_path = os.path.join(repo_root, "data", "last", "connected_output.csv")
    with open(output_path, mode="w", newline="") as file:
        ...
        for index, value in enumerate(total_H, start=1):
            writer.writerow([f"Iteration {index}: {value}"])
        writer.writerow([])
        writer.writerow(["Connectivity Maintenance Baseline"])
        writer.writerow([])
        writer.writerow([f"Ideal Communication Range: {Rc}"])
